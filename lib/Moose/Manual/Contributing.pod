...
This namespace is being phased out now that we are using GitHub's pull requests in our L</Development Workflow>.
...
Once your fork has been created, switch to your local working repo directory and update your C<origin> remote's push URL.  This allows you to use a single remote (C<origin>) to both pull in the latest code from GitHub and also push your work to your own fork:
...
Now, you're ready for action!  From now on, you just follow the L</Development Workflow> to publish your work and B<submit pull requests> to the Moose Cabal.
...
=item 1. Update your local repo with the latest from the official repo
...
review and follow the steps in the previous section, L</Getting Started> if you have not already.
...
use concise, descriptive branchnames such as: pod-syntax-contrib,
...
history or breaking fast-forward.  Useful command of this type are listed
below but please make sure that you understand what they do as they can be
destructive:
...
Doing so will greatly increase the chances of a speedy review and
acceptance into the mainline development.

=head3 Rebase on the Latest

Before your final push and issuing a pull request, you need to ensure that
your changes can be easily merged into the master branch of the upstream repository.  This is done by once again rebasing your branch on the latest C<origin/master>.
...
Now it's time to make your final push of the branch to your fork.  The C<--force> flag is only necessary if you've pushed before and subsequently rewritting your history:
