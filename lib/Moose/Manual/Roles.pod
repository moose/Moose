...
between two roles. If two roles both implement overloading for a given
operator, that is a conflict. If two roles both implement overloading and have
different C<fallback> values, that is also considered a conflict. These
conflicts are detected either when a role consumes another role or when
multiple roles are being composed into a class together.
