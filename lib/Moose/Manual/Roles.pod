...
It's possible to compose existing roles into new roles. For
example, we can have a C<HandleWithCare> class which applies both
the C<Breakable> and C<Fragile> roles to any class which consumes it:

  package HandleWithCare;

  use Moose::Role;

  with 'Breakable', 'Fragile';
